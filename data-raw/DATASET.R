...
# Variables retrievable by `pressurepath_create()`, per backend and per ERA5 product.
#
# These sets genuinely differ, and requesting a variable a product does not have used to return
# an empty response that silently blanked EVERY column (see GeoPressure/GeoPressureAPI#27).
# `pressurepath_variable_check()` validates against the resolved (source, era5_dataset) pair.
#
# GeoPressureAPI serves whatever bands the underlying Earth Engine collection carries:
#   `{js}` ee.ImageCollection("ECMWF/ERA5/HOURLY").first().bandNames()       -> single-levels
#   `{js}` ee.ImageCollection("ECMWF/ERA5_LAND/HOURLY").first().bandNames()  -> land
# "both" mosaics ERA5-Land on top of ERA5 but returns ERA5's band namespace, so its set equals
...
pressurepath_variable_api_single_levels <- c(
...
pressurepath_variable_api_land <- c(
  "dewpoint_temperature_2m",
